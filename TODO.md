...
- [ ] **CRITICAL: Phase 2 — GUI corner-to-corner polish.**
  ...
  - [~] **MEDIUM (2026-05-05, partial):** Dexie failure → toast bridge done. New `src/components/db/DexieErrorToastBridge.tsx` mounted next to `SyncToastBridge` in `App.tsx`. Listens to `db.on("versionchange")`, `db.on("blocked")`, and `window.unhandledrejection` filtered for Dexie/QuotaExceededError/InvalidStateError. Surfaces destructive toast with reload action; deduplicates by `name+message` so a runaway error loop doesn't spam. `syncService` errors already routed via `SyncToastBridge` (Phase 2B). **Still open:** per-route skeleton on first load (81 `useLiveQuery` call sites; switch `if (!data) return <p>Loading…</p>` → `<CardSkeleton/>` site by site).
