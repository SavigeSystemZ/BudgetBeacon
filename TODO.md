...
- [ ] CRITICAL: **Phase 2 — GUI corner-to-corner polish.**
  ...
  - [~] **MEDIUM (2026-05-05, in progress):** Dexie failure → toast bridge done (`src/components/db/DexieErrorToastBridge.tsx` mounted next to `SyncToastBridge`; versionchange / blocked / unhandledrejection filtered for Dexie/QuotaExceededError/InvalidStateError; dedup'd; reload action). Skeleton-on-first-load shipped on the 3 highest-traffic routes: **Dashboard** (`MetricSkeleton` × 4 + `CardSkeleton` × 4), **Ledger** (`CardSkeleton` + `TableRowSkeleton` × 5), **Reports** (header + 5-tab strip + `CardSkeleton`). Replaced bare `animate-pulse` text spinners with layout-matching skeletons + `role="status"` aria region. **Still open:** the other ~12 routes — Income, PayPath, StashMap, DebtCenter, Credit, TaxTaxi, DocumentStore, Subscriptions, Insurance, BeaconBridge, BudgetMissionControl, Settings — same pattern: `if (!data) return <Loading…/>` → matching `<CardSkeleton/>` block.
...
- [x] MEDIUM: **2026-05-02:** CouplesWealth read-only scavenger — dashboard charts + health card + ledger formatter; **MTD expense-by-category** on Reports (Monthly) and Mission Control via `ExpenseCategoryRollup` (`_system/context/scavenge-donor-couples-wealth.md`).
- [x] MEDIUM: **2026-05-02 (night batch):** Ledger **Top spend** + CSV **`expenseCategoriesMtd`**; **`MtdCategoryDonut`**; **`assistantContextFacts`** + **`collectAssistantPromptFacts`**; **`BeaconChatbot`** no-model MTD/insurance lines; **`TEST_STRATEGY`** Vitest+IDB note. **148** tests — see **`WHERE_LEFT_OFF.md`** for file list and validation.
...
- [x] MEDIUM: Streamed token rendering (M7.2 commit `114f8ef`).
- [x] MEDIUM: Tool-use / structured action proposals with explicit confirmation (M7.2 commit `114f8ef`).
- [x] MEDIUM: Provider health-check button in Settings (M7.2 commit `114f8ef`).
...
- [x] LOW: Decide on canonical Card primitive (`GlassCard` vs `Card`); migrate Settings to match the rest of the app.
...
- [x] MEDIUM: Mocked Bank Sync removed from `LedgerRoute.tsx`.
- [x] MEDIUM: Real CSV import — file picker → parse → map → dedupe → review → commit (M5).
- [x] MEDIUM: OFX/QFX parser shipped (M5 carry-over `dca6e70`) — bypasses column mapping.
- [x] MEDIUM: Merchant/payee normalization rules — `payeeRules` Dexie table + Settings panel + auto-apply during all imports (M5 carry-over `dca6e70`).
...
- [x] MEDIUM: **M10.4:** Relay + transport — thin Node+ws or Cloudflare Worker relay (~150 LOC). Client connects via `y-websocket` with the household-key envelope. "Sync status" indicator in bottom nav.
- [ ] MEDIUM: **M10.5:** Onboarding integration — existing `OnboardingWizard` learns to either continue local-only or sign up for sync. Migrate local Dexie data to the user's Household on first sync.
- [ ] MEDIUM: **M10.6:** Mobile parity smoke — real-device APK install, log in, verify data appears.

## M11 — Joint Household / Linked Accounts (queued — depends on M10)

- [ ] MEDIUM: **M11.1:** Invite flow — Settings → "Invite partner" generates a one-time code (24 h, single-use) wrapping an ephemeral key.
- [ ] MEDIUM: **M11.2:** Accept flow — second account enters code, gets the household key, joins.
- [ ] MEDIUM: **M11.3:** Per-record ownership UX — surface "owned by Person A / Person B / Joint" labels using existing `personId`. Add per-record visibility toggle (private vs shared).
- [ ] MEDIUM: **M11.4:** Activity log — new `householdActivity` Dexie table, populated from CRDT merge events.
- [ ] MEDIUM: **M11.5:** Leave / unlink — leaving account exports a v3 backup of their last state. Remaining account keeps the household; key rotates on leave.
- [ ] MEDIUM: **M11.6 (stretch):** View-only mode for a second account.
...
- [ ] MEDIUM: **Re-verify:** "M3 — Budget Engine." Reality: `calculateBudgetSummary` exists and is real aggregation, but no Vitest coverage exists. Re-mark only after M2 adds tests.
- [ ] MEDIUM: **Re-verify:** "M4 — Beacon Dashboard." Reality: Dashboard renders real data but has `alert("Snapshot saved.")` dead control. Re-mark after M3.
- [ ] MEDIUM: **Re-verify:** "M5 — Stash Map Forecasting." Reality: progress is `(current/target)*100`; no real forecasting. Re-scope or re-mark after M8.
- [ ] MEDIUM: **Re-verify:** "M6 — Credit Snapshot + Reports." Reality: Credit snapshots real, but Bank Fetch / Credit Check are `Math.random` mocks; Reports Export is `alert()`. Re-mark after M4 + M8.
- [ ] MEDIUM: **Re-verify:** "M7 — Backup + Polish + Hardening." Reality: backup export/import real, but no round-trip test, no error boundaries, no settings persistence. Re-mark after M2 + M4.
- [ ] MEDIUM: **Re-verify:** "M8 — Final Polish (Dark Mode + PWA)." Re-mark after M9 release-candidate pass.

## Genuinely-complete items (kept from prior TODO)

These are honest:
- [x] MEDIUM: Implemented Ledger & Transactions actual-spending tracker (manual entry).
- [x] MEDIUM: Fixed Android APK routing, assets, and VA seed data injection.
- [x] MEDIUM: Overhauled Mobile UX (Bottom Nav, Transparent Status Bar, Safe Areas) — re-verify safe areas in M9.
- [x] MEDIUM: Cross-platform Packaging (Android APK via Capacitor, Windows .exe config via Electron).
- [x] MEDIUM: Overhauled UI with "Immersive Deep Glass" aesthetic.
- [x] MEDIUM: Generated and exported VA Assistance Seed Data to `va-assistance-seed.json`.
- [x] MEDIUM: Tailored meta-system for Budget Beacon.
- [x] MEDIUM: Project Wrap-up. Git initialized.
- [x] MEDIUM: Advanced Installer. `install.sh` and install to `/home/whyte/.local/bin`.
- [x] MEDIUM: M2 — Income + Pay Path + Stash Map Forms (CRUD real).
- [x] MEDIUM: M1 — Data Model + Local Storage (Dexie v4 active).
- [x] MEDIUM: M0 — Scaffold Budget Beacon.
